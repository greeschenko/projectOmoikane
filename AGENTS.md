...
## Next Steps
1. **Phase 32** — Real events live: wire domain-event consumers (`cmd/audit` switches from HTTP to Kafka consumer; documented event catalog + `docs/events.md`); outbox relays already publish user.registered/page.published/post.published/media.uploaded (see [PLAN.md](./PLAN.md); `TODO.md` for phase checklist)
...
- **Go tests**: green via `make go-test` — 176 tests (112 handler + 10 events + 5 cmd/auth + 6 cmd/content + 5 cmd/media + 6 cmd/messages + 3 cmd/settings + 8 cmd/trash + 5 cmd/dashboard + 2 cmd/docs + 9 middleware + 2 mailer + 3 database). Phase 29 adds `TestNewEventID`, handler outbox-enqueue tests and `cmd/auth` service tests; Phase 30 adds 11 handler outbox tests (`content_outbox_test.go`) + `cmd/content` + `cmd/media` service tests; **Phase 31 adds 27 tests** — `cmd/messages` (6), `cmd/settings` (3), `cmd/trash` (8, fake upstreams + admin gate + routing + 502), `cmd/dashboard` (5, fake stats upstreams + shape merge + 502), `cmd/docs` (2), plus `Test*Service_InternalEndpoints` in cmd/auth/content/media (token gate + stats shape + TrashEntities scoping). dashboard_test.go/audit_test.go were DELETED with the monolith (dashboard/audit-logs now facade/audit-service). Need running PostgreSQL (`omoikane_test`); service tests mint JWTs via `auth.GenerateToken` as the `session` cookie.
- **Desktop Playwright**: 276/276 pass, 8 skipped — 0 failures (Phase 31 gate green with the fully-decomposed gateway; no e2e spec changes — routes now flow through services; Phase 26 added 4 tests: drag reorder, blog chips/filter, blog detail chips, api-tokens hydration-race guard; Phase 24 added a11y spec)
- **Mobile Playwright**: 275/275 pass, 9 skipped — 0 failures (Phase 31 gate green; Phase 26 adds drag reorder, blog chips/filter, blog detail chips; Phase 24 added a11y spec)
