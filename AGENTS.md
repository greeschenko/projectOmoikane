...
## Next Steps
1. **Phase 34** — Observability & ops hardening (probes everywhere, Prometheus `/metrics`, OTel tracing optional, JSON logs, migration Jobs, secrets via values, HPA, resource requests/limits) — see [PLAN.md](./PLAN.md); `TODO.md` for phase checklist
...
- **K8s gate (Phase 33)**: `make k8s-up` starts minikube (docker driver) + enables ingress/metrics-server/storage-provisioner, builds `omoikane/backend:phase33` + `omoikane/frontend:phase33`, `minikube image load`-s them, then `helm upgrade --install omoikane charts/omoikane -f charts/omoikane/values-minikube.yaml` and waits for all rollout statuses. Gateway NodePort `http://$(minikube ip):30080` (frontend role via `GATEWAY_PORT=30080`). `make k8s-test` = helm lint/template → **`k8s-db-reset` (scales the 6 DB-owning deployments to 0 → DROP DATABASEs via pg_terminate_backend → scale back to 1 → fresh processes re-run migrations)** → Playwright desktop → `k8s-db-reset` → Playwright mobile. Chart uses name byte-identical deployments/services to compose (DSNs, `REDIS_URL`, `KAFKA_BROKERS`, nginx upstreams are verbatim). KRaft quorum MUST be `KAFKA_CONTROLLER_QUORUM_VOTERS=1@localhost:9093` (dialing its own ClusterIP through kube-proxy crash-loops) and `rollout restart audit-service` after the broker recovers (consumer wedges if it starts during a Kafka outage). **Prod-standalone frontend quirks surface ONLY via the k8s gate** (compose `next dev` hides them): prerender/cache of SSR routes (`sitemap.ts` → `force-dynamic`), MUI `createSvgIcon` NOT emitting `data-testid` in production (explicit testids needed), and the mobile `useMediaQuery`—SSR-hydration race (admin drawer collapses → sidebar nav-link tests must skip on mobile).
...
### Documentation
- `AGENTS.md`: This file
- `TODO.md`: Phase 33 completed; Phase 34 next; i18n in Backlog
