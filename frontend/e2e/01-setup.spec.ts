...
test.describe("setup", () => {
  test.beforeEach(async ({}, testInfo) => {
    if (isMobile() && testInfo.title !== "submits setup form and redirects to /login") {
      // On mobile, only run the submission test to create admin user.
      // All other setup tests (redirect, form fields, validation) are
      // already covered by desktop and are not needed on mobile.
      test.skip();
    }
  });
