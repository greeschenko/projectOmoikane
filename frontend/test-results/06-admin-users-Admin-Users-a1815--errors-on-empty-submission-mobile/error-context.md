...
- Name: 06-admin-users.spec.ts >> Admin Users >> User creation form >> shows validation errors on empty submission
- Location: e2e/06-admin-users.spec.ts:67:9
...
Locator: getByText(/required|cannot be empty/i).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByText(/required|cannot be empty/i).first()
